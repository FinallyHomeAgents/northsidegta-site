...
import TownHeroCtas from "./TownHeroCtas";
...
export default function TownPageLayout({
  ...
  const hasSpotlight = Boolean(
    spotlightData?.loaded &&
      Array.isArray(spotlightData.items) &&
  ...
  return (
    <main className="flex-1">
      ...
      <section className="relative isolate overflow-hidden">
        ...
        <div className="relative z-10 mx-auto max-w-6xl px-4 py-12 sm:py-16 lg:py-20 md:min-h-[420px] lg:min-h-[520px]">
          <div className="grid items-start gap-8 lg:grid-cols-[minmax(0,2fr)_minmax(0,1.15fr)]">
            <div className="space-y-6 text-white">
              ...
              <TownHeroLifestyleCard {...(heroLifestyleCard || {})} townName={townName} />
            </div>
            {hasSpotlight && (
      ...
      <TownHeroCtas
        townName={townName}
        primaryHref={primaryButton?.href || "/buyers"}
        primaryLabel={primaryButton?.label || `Explore Homes in ${townName}`}
        secondaryHref="/about"
      />

      {hasSpotlight && (
        <section className="mx-auto mt-4 max-w-6xl px-4 md:hidden">
