...
const Styles = () => (
  ...
  .tooltip {
    position:absolute; z-index:30; backdrop-filter: blur(8px);
    background: rgba(255,255,255,.95); border: 1px solid rgba(16,185,129,.15);
    border-radius: 16px; box-shadow: 0 12px 28px rgba(2,44,34,.18);
    transform-origin: top left; transition: opacity .18s ease, transform .18s ease;
  }
  .tooltip-arrow {
    position:absolute; left:14px; width:14px; height:14px; transform: rotate(45deg);
    background: rgba(255,255,255,.95); border-left: 1px solid rgba(16,185,129,.15);
    border-top: 1px solid rgba(16,185,129,.15); backdrop-filter: blur(8px);
  }
        ...
        <div className="relative mx-auto mt-6 mb-0 rounded-2xl bg-white/70 p-3 shadow-sm border">
          ...
          <div className="absolute top-3 left-3 right-3 bottom-3 rounded-xl pointer-events-none">
            {TOWNS.map((t) => {
              ...
              const tipTop  = placeAbove ? clamp(t.y - yOffset, 6, 98) : clamp(t.y + yOffset, 2, 98);
                            ...
                            <div key={k} className="flex items-center justify-between">
                              ...
                              <div className="flex items-center gap-1.5">
                                {Array.from({ length: 5 }).map((_, i) => (
                                  <span
                                    key={i}
                                    className={`h-2 w-2 md:h-2.5 md:w-2.5 rounded-full ${i < Math.round(t.ratings[k]) ? "bg-emerald-600" : "bg-gray-300"}`}
                                  />
                                ))}
                              </div>
                            </div>
