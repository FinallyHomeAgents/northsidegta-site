...
function key(env) {
  if (!env.LIFE_CONNECTION_KEY && env.LIFE_META_APP_SECRET) return crypto.createHash('sha256').update('northside-life:connection-encryption:v1:' + env.LIFE_META_APP_SECRET).digest()
...
export function metaConfigured(env) {
  try {
    ...
    return Boolean(env.LIFE_META_APP_ID && env.LIFE_META_APP_SECRET && /^v\d+\.\d+$/.test(env.LIFE_META_GRAPH_VERSION || '') && url.protocol === 'https:' && url.pathname === '/api/life' && url.search === '?action=meta-callback' && !url.hash && !url.username && !url.password)
  } catch { return false }
}
...
export async function metaAction(req, res, { action, query, body, user, env, store }) {
  ...
  if (action === 'meta-start' && req.method === 'POST') {
    ...
    Object.entries({ client_id: env.LIFE_META_APP_ID, redirect_uri: env.LIFE_META_REDIRECT_URI, ...(env.LIFE_META_CONFIG_ID ? { config_id: env.LIFE_META_CONFIG_ID, override_default_response_type: 'true' } : { scope: permissions.join(','), auth_type: 'rerequest' }), response_type: 'code', state }).forEach(([k,v]) => url.searchParams.set(k,v))
