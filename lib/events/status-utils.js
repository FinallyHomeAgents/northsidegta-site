...
function resolveStatusForSync({
  ...
  if (sourceType === 'feed') {
    if (preservedStatus === 'published' || incomingStatus === 'published') {
      return 'published'
    }

    if (preservedStatus === 'approved' || incomingStatus === 'approved') {
      return 'approved'
    }

    if (preservedStatus === 'draft' || incomingStatus === 'draft') {
      return 'draft'
    }

    if (preservedStatus === 'pending' || incomingStatus === 'pending') {
      return 'pending'
    }

    return defaultNormalized === 'draft' ? 'draft' : 'pending'
  }
