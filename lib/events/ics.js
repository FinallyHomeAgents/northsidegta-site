...
function aggregateGroup(group, feed, now) {
  ...
  for (const occurrence of deduped) {
    ...
    if (!scheduleMap.has(isoDate)) {
      scheduleMap.set(isoDate, { date: isoDate, all_day: false, start: null, end: null })
    }
    ...
    if (!entry.start || occurrence.start < entry.start) {
      entry.start = occurrence.start
    }
    if (!entry.end || occurrence.end > entry.end) {
      entry.end = occurrence.end
    }
  }
  ...
  const schedule = Array.from(scheduleMap.values())
    .map((entry) => {
      if (entry.all_day) {
        return { date: entry.date, all_day: true, start_time: '', end_time: '' }
      }
      ...
      return {
        date: entry.date,
        all_day: false,
        start_time: entry.start.toFormat('HH:mm'),
        end_time: entry.end.toFormat('HH:mm'),
      }
    })
    .filter((entry) => entry && (entry.all_day || (entry.start_time && entry.end_time)))
