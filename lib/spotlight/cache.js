const { getRedisClient, isRedisConfigured } = require('../communityRanking/kv-client.js')
...
async function saveTownSpotlightData(townSlug, items = []) {
...
async function loadTownSpotlightData(townSlug) {
  const slug = normalizeSlug(townSlug)
  if (!slug) return null
...
function clearTownSpotlightMemoryCache() {
  memoryCache.clear()
}
