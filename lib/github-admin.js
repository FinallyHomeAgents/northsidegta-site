...
import { sanitizeEventId } from './admin-events.js'
...
function parseRepoParts(owner, repo) {
...
function resolveRepoFromEnv() {
  for (const envName of REPO_ENV_CANDIDATES) {
    const candidate = parseRepoString(process.env[envName])
    if (candidate) return candidate
  }

  for (const [ownerKey, repoKey] of REPO_ENV_PAIRS) {
    const candidate = parseRepoParts(process.env[ownerKey], process.env[repoKey])
    if (candidate) return candidate
  }
...
function resolveTokenFromEnv() {
  for (const envName of TOKEN_ENV_CANDIDATES) {
...
export function getGithubSyncCapability() {
  const token = resolveTokenFromEnv()
  const repo = resolveRepoFromEnv()

  if (token && repo) {
    return { ok: true, hint: '', token, repo }
  }
  ...
  if (!token && !repo) {
    return {
      ok: false,
      hint:
        'Sync is not configured — set GITHUB_TOKEN (or GH_TOKEN) and provide repository details via GITHUB_REPO or GITHUB_REPOSITORY.',
      token: '',
      repo: null,
    }
  }
  ...
  if (!token) {
    return {
      ok: false,
      hint:
        'Sync is not configured — missing GitHub token. Set GITHUB_TOKEN with Actions: write access (GH_TOKEN is also supported).',
      token: '',
      repo,
    }
  }
  ...
  return {
    ok: false,
    hint:
      'Sync is not configured — missing repository metadata. Set GITHUB_REPO=owner/repo or define GITHUB_REPOSITORY (or matching owner/name vars).',
    token,
    repo: null,
  }
}
