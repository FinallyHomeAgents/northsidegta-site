...
async function simpleHtmlList(feed, context) {
  ...
  const nodes = root.querySelectorAll(itemSelector).slice(0, config.maxItems || DEFAULT_MAX_HTML_ITEMS)

  for (const node of nodes) {
    const titleNode = config.titleSelector ? node.querySelector(config.titleSelector) : findFirst(node, ['h3 a', 'h2 a', 'h3', 'h2', '.event-title a', '.event-title'])
    if (!titleNode) continue

    const linkNode = titleNode.getAttribute('href') ? titleNode : titleNode.querySelector('a') || titleNode
    const rawLink = linkNode.getAttribute('href') || ''

    const summaryNode = config.summarySelector ? node.querySelector(config.summarySelector) : node.querySelector('p, .excerpt, .summary')
    const dateNode = config.dateSelector ? node.querySelector(config.dateSelector) : node.querySelector('time, .event-date')
    const timeAttr = dateNode?.getAttribute('datetime')
    const dateText = timeAttr || dateNode?.text || ''

    const locationNode = config.locationSelector ? node.querySelector(config.locationSelector) : node.querySelector('.event-location, .location, address')
    const imageNode = config.imageSelector ? node.querySelector(config.imageSelector) : node.querySelector('img')

    const start = parseDateFromText(dateText, config)
    if (!start) continue

    items.push({
      title: titleNode.text?.trim() || titleNode.innerText?.trim(),
      summary: summaryNode?.text?.trim() || summaryNode?.innerText?.trim() || '',
      description: summaryNode?.innerText?.trim() || '',
      start,
      end: parseDateFromText(dateNode?.getAttribute('data-end') || config.endDateSelector && node.querySelector(config.endDateSelector)?.text, config) || '',
      url: context.resolveUrl(rawLink),
      locationName: locationNode?.text?.trim() || '',
      image: imageNode?.getAttribute('src') ? context.resolveUrl(imageNode.getAttribute('src')) : '',
