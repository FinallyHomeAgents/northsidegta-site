...
function loadAllEventsFromDisk() {
...
function loadPendingEventsFromDisk() {
...
function getTorontoDayStart(reference) {
...
function isEventUpcoming(event, referenceStart) {
  const startDate = parseEventDate(event?.startDate ?? event?.start)
  if (!startDate) return true
  return startDate >= referenceStart
}

function isEventPast(event, referenceStart) {
  const startDate = parseEventDate(event?.startDate ?? event?.start)
  if (!startDate) return false
  return startDate < referenceStart
}

function filterEventsByScope(events, scope = 'upcoming', reference) {
...
function sortEventsByStartDate(events, direction = 'asc') {
