...
export default async function handler(req, res) {
  ...
  try {
    const result = await updateEventStatus(id, 'draft')
    ...
    console.error('[admin-events] unpublish failed', error)
    const status = error?.status && Number.isInteger(error.status) ? error.status : 500
    res.status(status).json({ ok: false, error: error?.message || 'Failed to unpublish event.' })
  }
}
